...
from utils.xcoms import xcom_pull
default_args = {
...
with DAG(dag_id="dag-3.2-example", default_args=default_args, schedule_interval="@daily") as dag:
    ...
    job_task = MyAwesomeOperator(
        task_id=job_task_id,
        super_variable=42
    )

    job_2_task = MyAwesomeOperatorTemplated(
        task_id="super_awesome_xcom_operator",
        templated_variable=xcom_pull(job_task_id)
        # templated_variable="{{ task_instance.xcom_pull('super_awesome_python_operator', key='return_value') }}"
    )
    ...
    start_task >> job_task >> job_2_task>> end_task
