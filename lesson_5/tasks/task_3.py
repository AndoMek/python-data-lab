...
def get_exchange_rate(cur_id: int, data_from: str = None, date_to: str = None):
    try:
        if data_from and date_to:
            data = requests.get(
                f"https://www.nbrb.by/API/ExRates/Rates/Dynamics/{cur_id}?startDate={data_from}&endDate={date_to}")
        ...
        raise requests.exceptions.SSLError(f"No access to NBRB")
